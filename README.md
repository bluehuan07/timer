...
### 2026-06-11（夜）
- 修正「閃光」設定項前的圓點不顯示、且文字位置往左偏的 bug：圓點原本的 class `dot flash` 撞到全螢幕閃光遮罩的 `.flash` 規則（`position:fixed; opacity:0`），導致圓點變透明又脫離排版。改名為 `dot light` 避開衝突。
- 圓點顏色設為暖黃 `#f5c84b`，與「專注／休息／組數／聲音」幾顆有色圓點型式統一。
- 快取版本升至 `timer-v12`。

### 2026-06-11（傍晚）
- 修正設定面板在小螢幕手機上會被裁掉、最底下「閃光」開關看不到也捲不到的問題。
- 設定面板改成置中浮層（與「安裝教學」一致）：卡片限制高度 `82dvh`、內容過長時可上下捲動，點暗幕即可關閉。
- 快取版本升至 `timer-v11`。
